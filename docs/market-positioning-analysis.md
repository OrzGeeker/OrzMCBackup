...
新格式）的世界按「玩家活跃时间（InhabitedTime）」逐区块裁剪，把 18.6GB 的世界压到 2.2GB，作为优化
...
一个中小型 PaperMC 服务端轻松膨胀到 **14~19GB**（本项目真实验证数据：18.6GB / 28,451 文件 /
21,688 个 `.mca` / 3,162,906 区块）。

- **存储成本线性增长**：备份存储、异地灾备、对象存储都按体积付费；
- **备份耗时长**：全量备份 18GB，频率越高越吃带宽与磁盘 IO；
- **恢复慢**：灾难恢复时从远程拉取 18GB 级别的包代价高。

**解决效果（真实数据）**：`threshold-benchmark-report.md` 记录了 18.6GB 恢复合并世界经
`-t 300`（5 分钟）优化后降至 **2.18GB（-88.3%）**，剔除 92.7% 区块，且非预期丢失文件数为 **0**
（六档阈值 0/60/120/180/240/300s 逐字节验证）。**60 秒阈值是收益拐点**——从 t0 到 t60 剔除率
71.3%→91.3%（输出 6.9GB→2.6GB），继续提高阈值边际收益很小（t300 仅再多剔除 1.4%）。
